...
async def example_2_create_profile_embeddings():
    ...
    result = await call_mcp_tool(
        "create_embeddings",
        {
            ...
            "metadata": {"source": "resume", "name": "John Doe", "date": "2024-01-15"},
        },
    )
...
async def example_6_complete_workflow():
    ...
    extracted = await call_mcp_tool(
        "extract_content", {"file_type": "pdf", "content": "/path/to/resume.pdf"}
    )

    # Step 2: Create embeddings
    print("\nStep 2: Create profile embeddings")
    embeddings = await call_mcp_tool(
        "create_embeddings",
        {
    ...
    print("\nStep 3: Extract job description")
    job_desc = await call_mcp_tool(
        "extract_content", {"file_type": "web", "url": "https://example.com/job"}
    )
...
async def example_7_with_metadata_filtering():
    ...
    result = await call_mcp_tool(
        "query_embeddings",
        {
            ...
            "collection_name": "profile",
            "top_k": 5,
            "filter_metadata": {"source": "resume", "section": "experience"},
...
async def example_8_multiple_providers():
    ...
    ollama_result = await call_mcp_tool(
        "create_embeddings",
        {
            "content": content,
    ...
    openai_result = await call_mcp_tool(
        "create_embeddings",
        {
            "content": content,
